...
-- Find each customers first order (earliest order date)
-- Check if the order is immediate (order_date = customer_pref_delivery_date)
...
SELECT 
ROUND(100.00 * SUM(order_date = customer_pref_delivery_date)/COUNT(*),2) AS immediate_percentage
FROM first_order
WHERE rn = 1